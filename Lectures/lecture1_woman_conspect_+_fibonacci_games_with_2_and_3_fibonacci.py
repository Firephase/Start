# -*- coding: utf-8 -*-
"""Lecture1_woman_conspect + fibonacci games with 2- and 3-fibonacci.ipynb
...
def fib3_period(n):
  ...
  while not( arr[s-1][0] == arr[0][0] and arr[s-1][1] == arr[0][1] and arr[s-1][2] == arr[0][2]):
    arr[s][0] = arr[s-1][1]
    arr[s][1] = arr[s-1][2]
    arr[s][2] = (arr[s-1][0] + arr[s-1][1] + arr[s-1][2])%n
   #print(arr[s])
    s += 1
  #print(arr,sep='\n')
  #print(s - 1)
...
for i in range(n//5,n):
    if is_prime(i):
      x.append(i)
      y.append(fib3_period(i)/i)



#print(x)
#print(y)
print('fib3_period(p)/p^2 ~ ', max(x,y))

plt.plot(x,y)
#plt.ylabel(y)
plt.show()

print(fib3_period(3),fib3_period(4),fib3_period(12))